...
class GMM:
    ...
    def _e_step(self, train_X):
        ...
        for i in range(N):
            ...
            for j in range(self.K):
                xd = train_X[i] - self.centers[j]
                det = np.linalg.det(self.covariances[j])
                prob[j] = self.mixture_proportions[j] * (1/(np.power(((2*np.pi)**D)*det, 0.5))) * np.exp(-0.5 * xd@np.linalg.inv(self.covariances[j])@xd)
            for j in range(self.K):
                probability_matrix[i][j] = prob[j]/np.sum(prob)
        # ====================================================
    ...
    def _m_step(self, train_X, probability_matrix):
        ...
        for k in range(self.K):
            temp = np.zeros((1, D))
            for i in range(N):
                temp += probability_matrix[i][k]*train_X[i]
            centers[k] = temp/sum(probability_matrix[:, k])
            mixture_proportions[k] = sum(probability_matrix[:, k])/N
            covariances[k] = ((train_X-centers[k]).T@(probability_matrix[:, k:k+1] * (train_X-centers[k])))/sum(probability_matrix[:, k])
        # ====================================================
